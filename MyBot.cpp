...
int main(int argc, char* argv[]) {
    ...
    for (;;) {
        ...
        for (const auto& ship_iterator : me->ships) {
			...
			log::log("Ship " +to_string(ship->id) + "has Halite " +to_string(ship->halite));

			if (ship->shipState == eShipState::eUnkonown) {
				ship->shipState = eShipState::eExploring;
			}
			
			if (ship->shipState == eShipState::eReturning) {
				...
				ship->shipState = eShipState::eReturning;
			}
